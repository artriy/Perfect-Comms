...
- **Wine / Linux:** the existing `start.exe /unix /bin/sh -c ...` supervisor creates a private mode-`0700` directory, publishes the mode-`0600` token, validates the helper contract, starts the native helper, and owns its exit receipts. Linux retains its staged `/tmp` helper plus original-path fallback for `noexec` mounts.
- **CrossOver / macOS:** Windows-target .NET ZIP extraction under CrossOver does not restore the Mach-O executable bit, and `start.exe /unix <app>` treats the bundle as a directory. The managed launcher therefore publishes an owned, nonce-named request beside the content-validated signed bundle, dynamically resolves Wine's direct Unix process bridge, runs positional `/bin/chmod u+x` plus targeted quarantine removal, and dispatches `/usr/bin/open -g -j -n PerfectCommsAudio.app` without a shell or Terminal. The argument-free app broker validates request ownership and modes, creates the private directory and token slot, then starts and supervises an argument-bearing child of its own bundle executable. Concurrent broker instances claim each request atomically; the exact nonce receipt remains authoritative.
- **Host-exec blocked:** the receipt or handshake times out; the `CaptureSupervisor` retries within its restart budget and, if the helper never starts, enters the all-failed "voice unavailable" state (logged). There is no in-proc desktop fallback since BASS was removed in 4.0.
