...
- **CrossOver / macOS:** CrossOver's Unix-file association does not reliably preserve arguments to `/bin/sh`; it can open an interactive Terminal and discard `-c` plus the bootstrap script. The managed launcher therefore publishes an owned, nonce-named request beside the signed bundle and opens only `PerfectCommsAudio.app` through `start.exe /unix`. The argument-free app broker validates request ownership and modes, creates the private directory and token slot, then starts and supervises an argument-bearing child of its own bundle executable. It remains alive while children or bootstrap ownership guards exist, so LaunchServices single-instance activation can service overlapping enumeration and voice requests without Terminal.
