...
/// Owns the callback box until libcubeb has accepted the stream and every optional callback has
/// been registered. On success ownership moves to `Stream`; on any earlier return this guard frees
/// the box after the partially-created native stream (if any) has been destroyed.
struct StreamCallbacksOwner<F> {
    ptr: *mut StreamCallbacks<F>,
}

impl<F> StreamCallbacksOwner<F> {
    fn new(callbacks: StreamCallbacks<F>) -> Self {
        Self {
            ptr: Box::into_raw(Box::new(callbacks)),
        }
    }

    fn as_ptr(&self) -> *mut StreamCallbacks<F> {
        self.ptr
    }
...
impl<F> Drop for StreamCallbacksOwner<F> {
    fn drop(&mut self) {
        if !self.ptr.is_null() {
            // SAFETY: `ptr` came from `Box::into_raw`, is uniquely owned by this guard, and is
            // cleared before ownership transfers to a successfully-created `Stream`.
...
impl<'a, F> StreamBuilder<'a, F> {
    ...
    pub fn init(self, ctx: &ContextRef) -> Result<Stream<F>> {
        ...
        // This is the low-level stream: on registration failure it destroys the native stream,
        // then `callbacks` frees the still-unclaimed user pointer. Only `Stream::new` below moves
        // callback cleanup into the high-level stream's Drop implementation.
        let stream: cubeb_core::Stream = unsafe {
            ctx.stream_init(
                stream_name,
                input_device,
                input_stream_params,
...
mod tests {
    ...
    fn callback_owner(drops: Arc<AtomicUsize>) -> StreamCallbacksOwner<MonoFrameForTest> {
        let signal = DropSignal(drops);
        StreamCallbacksOwner::new(StreamCallbacks {
            data: Box::new(move |_, _| {
                let _ = &signal;
                0
            }),
            state: Box::new(|_| {}),
    ...
    #[derive(Clone, Copy)]
    struct MonoFrameForTest;

    #[test]
    fn callback_owner_frees_callbacks_on_early_return() {
        let drops = Arc::new(AtomicUsize::new(0));
        drop(callback_owner(drops.clone()));
        assert_eq!(drops.load(Ordering::Relaxed), 1);
    }

    #[test]
    fn callback_owner_release_transfers_ownership() {
        let drops = Arc::new(AtomicUsize::new(0));
        let owner = callback_owner(drops.clone());
        let ptr = owner.as_ptr();
        owner.release();
        assert_eq!(drops.load(Ordering::Relaxed), 0);
        // SAFETY: this test assumes the ownership that `Stream::drop` owns in production.
